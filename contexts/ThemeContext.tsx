...
export const SECURITY_STYLES: Record<string, {
    container: string;
    text: string;
    subText: string;
    keypad: string;
...
const getNoteColor = (color: string, themeId: string, isDark: boolean): string => {
   const palette = NOTE_COLORS[color];
   if (!palette) return isDark ? NOTE_COLORS.default.dark : NOTE_COLORS.default.classic;
...
interface ThemeContextType {
  ...
  addCustomTheme: (name: string, base: 'light'|'dim'|'dark', accent: string) => void;
...
export const ThemeProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    if (savedCustom) {
        try { setCustomThemes(JSON.parse(savedCustom)); } catch(e) {}
    }
  ...
  const addCustomTheme = (name: string, base: 'light'|'dim'|'dark', accent: string) => {
  ...
  const getNoteColorStyle = (color: string) => getNoteColor(color, themeId, activeStyles.isDark);
