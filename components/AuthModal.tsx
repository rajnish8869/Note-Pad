    ...
    <div className={`fixed inset-0 z-[100] flex flex-col pt-[env(safe-area-inset-top)] pb-[calc(2rem+env(safe-area-inset-bottom))] select-none min-h-[100dvh] ${s.container}`}>
        <div className="flex-1 flex flex-col items-center justify-center">
            ...
            <div className={`flex gap-6 h-4 items-center justify-center mb-8 ${error ? "animate-[shake_0.5s_ease-in-out]" : ""}`}>
                {Array.from({ length: targetLength > 0 ? targetLength : Math.max(4, pin.length) }).map((_, i) => (
                    <div key={i} className={`w-4 h-4 rounded-full border-2 transition-all duration-300 ${i < pin.length ? s.dotActive : s.dotInactive}`} />
                ))}
            </div>
        ...
        <div className="px-8 pb-4 w-full max-w-sm mx-auto">
            <div className="grid grid-cols-3 gap-x-8 gap-y-6">
                {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(n => (
                    <button key={n} onClick={() => handleNumClick(n.toString())} className={`w-20 h-20 mx-auto rounded-full text-3xl font-light flex items-center justify-center transition-all active:scale-95 outline-none select-none ${s.keypad}`}>{n}</button>
                ))}
                <div className="w-20 h-20 mx-auto flex items-center justify-center">
                   {onCancel && <button onClick={onCancel} className={`text-xs font-bold tracking-widest uppercase transition-colors py-4 ${s.keypadAction}`}>Cancel</button>}
                </div>
                <button onClick={() => handleNumClick("0")} className={`w-20 h-20 mx-auto rounded-full text-3xl font-light flex items-center justify-center transition-all active:scale-95 outline-none select-none ${s.keypad}`}>0</button>
                <button onClick={() => setPin(p => p.slice(0, -1))} className={`w-20 h-20 mx-auto rounded-full active:scale-90 flex items-center justify-center transition-all outline-none ${s.keypadAction}`}><Icon name="arrowLeft" size={28} /></button>
            </div>
        </div>
    </div>
    );
