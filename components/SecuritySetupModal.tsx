...
export const SecuritySetupModal: React.FC<Props> = ({
  onComplete,
  onCancel,
}) => {
  ...
  return (
    ...
    >
      <div className="flex-1 flex flex-col items-center justify-center">
        <div
          className={`mb-6 transition-all ${step === 2 ? "scale-110" : ""} ${
            step === 1 ? s.icon : "text-green-500"
          }`}
        >
        ...
        <p
          className={`text-sm font-medium tracking-wide uppercase transition-all ${
            error ? "text-red-500" : s.subText
          }`}
        >
        ...
        <div className="grid grid-cols-3 gap-x-8 gap-y-6">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map((n) => (
            <button
              ...
              className={`w-20 h-20 mx-auto rounded-full text-3xl font-light flex items-center justify-center transition-all active:scale-95 outline-none ${s.keypad}`}
            >
              {n}
            </button>
          ))}
