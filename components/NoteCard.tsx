      ...
      ? theme === "neo-glass"
        ...
        : `border-${
            styles.accentColor === "#2563eb"
              ? "blue"
              : styles.accentColor.replace("#", "")
          } shadow-md ring-1 ${styles.primaryRing} ring-opacity-20`
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <h3
          className={`font-bold text-lg leading-tight flex-1 pr-4 ${
            !note.title && isLocked ? "italic opacity-50" : ""
          }`}
        >
        ...
        {selectionMode ? (
          <div
            className={`flex-shrink-0 transition-colors ${
              isSelected ? styles.primaryText : styles.secondaryText
            }`}
          >
            <Icon
          ...
          note.isPinned && (
            <div
              className={`absolute top-0 right-0 p-2.5 rounded-bl-2xl shadow-sm ${styles.fab}`}
            >
      ...
      {isLocked ? (
        <div
          className={`mt-3 h-20 rounded-2xl flex flex-col items-center justify-center gap-1 border-2 border-dashed transition-colors ${styles.lockedBg} ${styles.lockedBorder}`}
        >
          <div
            className={`p-1.5 rounded-full ${styles.tagBg} ${
              isCustomLock ? styles.primaryText : styles.secondaryText
            }`}
          >
        ...
        <p
          className={`text-[0.95rem] leading-relaxed line-clamp-[8] mb-3 min-h-[1.5rem] whitespace-pre-wrap opacity-90 ${
            !note.title ? "text-lg pt-1 font-medium text-gray-400" : ""
          }`}
        >
      ...
      {!isLocked && (hasImage || hasAudio || note.location) && (
        <div className="flex gap-3 mb-3 mt-2">
          {hasImage && (
            <div className={`p-1.5 rounded-lg ${styles.tagBg}`}>
              <Icon name="image" size={16} className={styles.secondaryText} />
            </div>
          )}
          {hasAudio && (
            <div className={`p-1.5 rounded-lg ${styles.tagBg}`}>
              <Icon name="mic" size={16} className={styles.secondaryText} />
            </div>
          )}
          {note.location && (
            <div className={`p-1.5 rounded-lg ${styles.tagBg}`}>
              <Icon name="mapPin" size={16} className={styles.secondaryText} />
            </div>
          )}
        </div>
      )}
      ...
      {note.tags && note.tags.length > 0 && !isLocked && (
        <div className="flex flex-wrap gap-1.5 mb-1 mt-auto pt-2">
          {note.tags.slice(0, 3).map((tag) => (
            <span
              key={tag}
              className={`text-[10px] font-bold tracking-wide px-2.5 py-1 rounded-md ${styles.tagBg} ${styles.tagText}`}
            >
              #{tag}
            </span>
          ))}
          {note.tags.length > 3 && (
            <span className={`text-[10px] self-center ${styles.secondaryText}`}>
              +{note.tags.length - 3}
            </span>
          )}
        </div>
      ...
      <div
        className={`flex justify-between items-center mt-3 pt-2 ${
          !isTrashView ? `border-t ${styles.divider} border-opacity-40` : ""
        } text-[10px] ${styles.secondaryText}`}
      >
        <span className="flex items-center gap-1.5 font-medium tracking-wide">
          ...
          {!isTrashView && note.isSynced && (
            <Icon name="check" size={12} className={styles.successText} />
          )}
          {note.isIncognito && (
            <Icon name="incognito" size={12} className="ml-1 opacity-70" />
          )}
        </span>
      </div>
      ...
      {!selectionMode && isTrashView && (
        <div
          className={`flex justify-end gap-3 mt-3 pt-2 border-t ${styles.divider}`}
        >
          <button
            onClick={onRestore}
