...
export const NoteListView: React.FC<Props> = ({
  ...
  const rawFilteredNotes = useMemo(() => {
    ...
    if (activeFilter !== "ALL") {
      ...
      if (activeFilter === "LOCKED")
        filtered = filtered.filter((n) => n.isLocked || n.isEncrypted);
      if (activeFilter === "MEDIA")
        filtered = filtered.filter((n) => n.hasImage);
      if (activeFilter === "AUDIO")
        filtered = filtered.filter((n) => n.hasAudio);
    }
  ...
  const { pinnedNotes, otherNotes, showPinnedSection } = useMemo(() => {
    const shouldSplit =
      view !== "TRASH" && activeFilter === "ALL" && sortBy === "UPDATED";
  ...
  const handleBulkPin = () => {
    ...
    const allPinned = ids.every(
      (id) => notes.find((n) => n.id === id)?.isPinned
    );
    ...
    <button
      ...
      className={`
            ...
            ${
              activeFilter === type
                ? theme === "vision"
                  ? "bg-[#2F6BFF] border-[#2F6BFF] text-white shadow-lg shadow-blue-500/20"
                  : `${styles.fab} border-transparent`
  ...
  return (
    <div className="w-full max-w-md md:max-w-4xl min-h-[100dvh] relative flex flex-col">
      ...
      <header
        className={`pt-[calc(1.5rem+env(safe-area-inset-top))] px-5 pb-2 flex flex-col gap-5 z-20`}
      >
        <div className="flex justify-between items-start">
          <div>
            <p
              className={`text-sm font-medium opacity-60 uppercase tracking-wide mb-1 ${styles.secondaryText}`}
            >
              {greeting},
            </p>
          ...
          <button
            onClick={onMenuClick}
            className={`relative w-12 h-12 rounded-full flex items-center justify-center overflow-hidden border-2 transition-all active:scale-95 shadow-sm ${
              theme === "neo-glass"
        ...
        <div className="flex gap-3">
          <div
            className={`flex-1 h-14 rounded-2xl flex items-center px-4 transition-all shadow-sm ${
              theme === "neo-glass"
                ? "bg-black/20 border border-white/10"
                : `${styles.cardBase} ${styles.cardBorder} border`
            }`}
          >
          ...
          <button
            ...
            className={`w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm transition-all active:scale-95 ${
              theme === "neo-glass"
      ...
      <main className="flex-1 px-4 pb-[calc(6rem+env(safe-area-inset-bottom))] mt-2">
        {view === "TRASH" && (
          <div
            className={`mb-6 py-3 px-4 rounded-xl text-xs text-center font-medium ${
              theme === "neo-glass"
                ? "bg-white/10 text-white/70 border border-white/10"
                : theme === "dark" || theme === "vision"
        ...
        {rawFilteredNotes.length === 0 ? (
          <div
            className={`flex flex-col items-center justify-center pt-20 ${styles.secondaryText}`}
          >
            <div
            ...
            >
              <Icon
                name={searchQuery ? "search" : "fileText"}
                size={48}
            ...
            <p className="text-xs opacity-60 mt-2">
              {searchQuery ? "Try a different keyword" : "Tap + to create one"}
            </p>
          </div>
        ) : (
          <>
            {showPinnedSection && (
              <div className="mb-8 animate-slide-in">
                <div
                  className={`text-xs font-bold uppercase tracking-wider mb-3 px-1 opacity-50 ${styles.text}`}
                >
                  Pinned
                </div>
                <div
                ...
                <div
                  className={`text-xs font-bold uppercase tracking-wider mt-6 mb-3 px-1 opacity-50 ${styles.text}`}
                >
                  Others
                </div>
              </div>
            ...
            {rows.others.length > 0 ? (
              ...
              !showPinnedSection && (
                <div className={`text-center py-10 opacity-50 ${styles.text}`}>
                  No other notes
                </div>
              )
            )}
      ...
      {selectionMode && (
        <div className="fixed bottom-6 left-4 right-4 z-40 animate-slide-up">
          <div
            className={`rounded-full shadow-2xl p-2 px-6 flex items-center justify-between backdrop-blur-md ${
              theme === "neo-glass"
                ? "bg-black/60 border border-white/20"
                : "bg-white/90 dark:bg-[#2c2c2c]/90 border border-gray-200 dark:border-gray-700"
            }`}
          >
            <button
            ...
            <div className="flex items-center gap-1">
              {view === "TRASH" ? (
                <>
                  <button
                    ...
                    className={`p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 ${styles.successText}`}
                    title="Restore"
                  >
                  ...
                  <button
                    ...
                    className={`p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 ${styles.dangerText}`}
                    title="Delete"
                  >
                ...
                <>
                  <button
                    ...
                    className={`p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 ${styles.text}`}
                    title="Move"
                  >
              ...
              <button
                ...
                className={`p-2 rounded-full hover:bg-black/5 dark:hover:bg-white/10 ${styles.text} ml-2`}
                title="Select All"
              >
                <Icon
                  name={
                    selectedIds.size === rawFilteredNotes.length
                      ? "checkCircle"
                      : "circle"
                  }
                  size={20}
        ...
        <div className="space-y-1">
          <button
            ...
            className={`w-full flex items-center justify-between p-4 rounded-xl ${
              sortBy === "UPDATED" ? styles.activeItem : styles.iconHover
            }`}
          >
            <span className={styles.text}>Last Modified</span>
            {sortBy === "UPDATED" && (
              <Icon name="check" size={20} className={styles.primaryText} />
            )}
          </button>
          <button
            onClick={() => {
            ...
            className={`w-full flex items-center justify-between p-4 rounded-xl ${
              sortBy === "CREATED" ? styles.activeItem : styles.iconHover
            }`}
          >
          ...
          <button
            ...
            className={`w-full flex items-center justify-between p-4 rounded-xl ${
              sortBy === "TITLE" ? styles.activeItem : styles.iconHover
            }`}
          >
          ...
          <div className="flex gap-2 p-1 bg-gray-100 dark:bg-gray-800 rounded-xl p-1">
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
                layoutMode === "GRID"
                  ? `${styles.cardBase} shadow-sm ${styles.text}`
      ...
      >
        <div className="space-y-2">
          <button
            ...
            className={`w-full flex items-center gap-3 p-4 rounded-xl border ${styles.buttonSecondary} border-transparent`}
          >
            <div className={`p-2 rounded-full ${styles.tagBg}`}>
          ...
          {folders.map((f) => (
            <button
              ...
              className={`w-full flex items-center gap-3 p-4 rounded-xl border ${styles.buttonSecondary} border-transparent`}
            >
              <div className={`p-2 rounded-full ${styles.primaryBg}`}>
