...
export class StorageService {
  ...
  private static async migrateLegacyNative(): Promise<void> {
    return new Promise((resolve) => {
      ...
      // 2. Event Listeners
      const batchListener = MigrationPlugin.addListener(
        "onNotesBatch",
        async (data) => {
          if (isResolved) return;
      ...
      const completeListener = MigrationPlugin.addListener(
        "onMigrationComplete",
        async () => {
      ...
      const errorListener = MigrationPlugin.addListener(
        "onMigrationError",
        async (err) => {
      ...
      MigrationPlugin.startLegacyMigration().catch((e) => {
        console.warn(
          "[StorageService] Native migration start failed (fallback)",
          e
        );
        clearTimeout(timer);
        finalize();
   ...
   */
  private static async processMigrationNoteContentOnly(
    note: Note
  ...
  private static async updateSearchIndex(
    id: string,
    text: string,
    isEncrypted: boolean
  ): Promise<void> {
  ...
  private static async saveSearchIndex(
  ...
  static async search(query: string): Promise<string[]> {
    ...
    return Object.keys(index).filter(
      (id) => index[id] && index[id].includes(lowerQuery)
    );
  }
  ...
  static async getNoteContent(id: string): Promise<string> {
    return (
      (await localforage.getItem<string>(`${NOTE_CONTENT_PREFIX}${id}`)) || ""
    );
  }
  ...
  static async saveNote(
    ...
    console.log(
      `[StorageService] saveNote ${note.id} | Content defined: ${
        note.content !== undefined
      } | EncryptedData defined: ${note.encryptedData !== undefined}`
    );
